...
public class APdu {
    ...
    private static APdu decode(byte[] bytes, boolean encrypt, byte[] serverSystemTitle, int frameCounter,
        ...
        if (tag >= 0x60 && tag <= 0x63) {
            ...
            if (encrypt) {
                if (acseAPdu.getAare() != null && acseAPdu.getAare().getRespondingAPTitle() != null) {
                    serverSystemTitle = acseAPdu.getAare().getRespondingAPTitle().getApTitleForm2().value;
                }
                else if (acseAPdu.getAarq() != null) {
                    ContextId contextId = ObjectIdentifier
                            .applicationContextIdFor(acseAPdu.getAarq().getApplicationContextName());
                    ...
                    if (acseAPdu.getAarq().getCallingAPTitle() != null) {
                        serverSystemTitle = acseAPdu.getAarq().getCallingAPTitle().getApTitleForm2().value;
        ...
        if (encrypt) {
            ...
            plaintext = GcmModule.decrypt(ciphertext, serverSystemTitle, securitySuite);
    ...
    private static void setDataToBuilder(APdu apdu, RawMessageDataBuilder rawMessageBuilder, byte[] ciphertext,
        ...
        try {
            if (plaintext == null && apdu.cosemPdu != null) {
