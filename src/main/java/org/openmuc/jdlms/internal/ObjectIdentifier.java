...
public class ObjectIdentifier {
    ...
    public static MechanismName mechanismNameFor(AuthenticationMechanism mechanismId) {
        return new MechanismName(identifierFor(ObjectIdentifierId.MECHANISM_NAME, mechanismId.getId()));
    }

    public static ApplicationContextName applicationContextNameFor(ContextId contextId) {
        return new ApplicationContextName(identifierFor(ObjectIdentifierId.CONTEXT, contextId.getCode()));
    }

    public static ContextId applicationContextIdFor(BerObjectIdentifier applicationContext) {
        return ContextId.contextIdFor(applicationContext.value[INDEX_OF_ID]);
    }

    private static int[] identifierFor(ObjectIdentifierId objectIdentifierId, int id) {
        return new int[] { JOINT_ISO_CCITT, COUNTRY, COUNTRY_NAME, IDENTIFIED_ORGANIZATION, DLMS_UA,
                objectIdentifierId.getId(), id };
    }

    public static AuthenticationMechanism mechanismIdFor(MechanismName mechanismName) {
