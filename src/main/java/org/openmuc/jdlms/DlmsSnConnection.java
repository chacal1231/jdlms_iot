...
class DlmsSnConnection extends DlmsConnection {
    ...
    private final SnQueue<ReadResponse> readResponseQueue;
    private final SnQueue<WriteResponse> writeResponseQueue;
    ...
    DlmsSnConnection(Settings settings, SessionLayer transportLayerCon, Map<ObisCode, SnObjectInfo> snObjectMapping)
        ...
        this.readResponseQueue = new SnQueue<>();
        this.writeResponseQueue = new SnQueue<>();
    ...
    public synchronized List<GetResult> get(boolean highPriority, List<AttributeAddress> params) throws IOException {
        ...
        while (addrIter.hasNext()) {
            ...
            try {
                ...
                res[nextIndex] = new GetResult(AccessResultCode.SCOPE_OF_ACCESS_VIOLATED);
            }
        }
        ...
        ReadResponse response = pollReadResponse();
    ...
    private ReadResponse pollReadResponse() throws IOException {
        ReadResponse response = readResponseQueue.poll();

        ReadResponse.SubChoice.Choices choiceIndex = response.get(0).getChoiceIndex();
        ...
        while (true) {
            Data_Block_Result dataBlockResult = response.get(0).data_block_result;

            baos.write(dataBlockResult.raw_data.getValue());

            if (dataBlockResult.last_block.getValue()) {
                response = new ReadResponse();
                response.decode(new ByteArrayInputStream(baos.toByteArray()));
                return response;
            }

            Unsigned16 blockNumber = dataBlockResult.block_number;

            requestNexBlock(blockNumber);

            response = readResponseQueue.poll();

        }

    }

    private void requestNexBlock(Unsigned16 blockNumber) throws IOException {
    ...
    public synchronized List<AccessResultCode> set(boolean highPriority, List<SetParameter> params) throws IOException {
        ...
        Iterator<WriteResponse.SubChoice> responseIter = writeResponseQueue.poll().iterator();
    ...
    private List<MethodResult> confirmedMethdoCall(List<MethodParameter> params) throws IOException {
        ...
        while (paramsIter.hasNext()) {
            ...
            SnObjectInfo snObjectInfo = accessSnObjectInfo(param);
        ...
        Iterator<ReadResponse.SubChoice> iter = pollReadResponse().list().iterator();
        int resId = 0;
        while (iter.hasNext()) {
    ...
    private SnObjectInfo accessSnObjectInfo(CosemResourceDescriptor cosemResourceDescriptor) throws IOException {
    ...
    private SnObjectInfo getVariableInfo(CosemResourceDescriptor param) throws IOException {
        ...
        ReadResponse response = this.readResponseQueue.poll();
    ...
    private class SnQueue<E extends AxdrType> {
        ...
        public E poll() throws IOException {
            try {
                E result = q.poll(connectionSettings().responseTimeout(), TimeUnit.MILLISECONDS);

                if (result == null) {
                    throw new ResponseTimeoutException("Request timed out..");
                }
